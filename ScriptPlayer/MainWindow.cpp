...
#include "../ServoAPI/MemeServoAPI.h"
...
void MainWindow::on_localError(unsigned char node_addr, unsigned char err)
{
    ...
    ui->plainTextEdit_Logs->appendPlainText(QDateTime::currentDateTime().toString("yyyy-MM-dd HH:mm:ss") + QObject::tr(" - API调用错误：0x%1，节点：0x%2").arg(err, 2, 16, QLatin1Char('0')).arg(node_addr, 2, 16, QLatin1Char('0')));
}
...
void MainWindow::on_nodeError(unsigned char node_addr, unsigned char err)
{
    ...
    ui->plainTextEdit_Logs->appendPlainText(QDateTime::currentDateTime().toString("yyyy-MM-dd HH:mm:ss") + QObject::tr(" - 节点0x%1错误：0x%2").arg(node_addr, 2, 16, QLatin1Char('0')).arg(err, 2, 16, QLatin1Char('0')));
}
...
void MainWindow::on_scriptLabel(int16_t currLabel)
{
    ...
    {
        QMessageBox msgBox;
        msgBox.setText(QObject::tr("脚本执行错误:'%1'").arg(currLabel));
        msgBox.exec();
    }
    else
    {
        QMessageBox msgBox;
        msgBox.setText(QObject::tr("脚本执行结束"));
        msgBox.exec();
    }
}
...
void MainWindow::on_pushButton_Port_clicked()
{
    ...
    else
    {
        ...
        {
            ui->pushButton_Port->setText(QObject::tr("关闭"));
            ...
            statusBar()->showMessage(QObject::tr("端口\"%1\"打开成功").arg(ui->comboBox_Port->currentText()), 5000);
        }
        else
            statusBar()->showMessage(QObject::tr("端口\"%1\"打开失败").arg(ui->comboBox_Port->currentText()), 5000);
    }
}
...
void MainWindow::on_pushButton_ScriptLoad_clicked()
{
    ...
    QString fileName = QFileDialog::getOpenFileName(this, QObject::tr("打开脚本文件"), "", QObject::tr("脚本文件 (*.txt)"));
    ...
    {
        msgBox.setText(QObject::tr("文件打开失败：'%1'").arg(fileName));
        msgBox.exec();
    ...
    if (!model)
    {
        ...
        model->setHeaderData(0, Qt::Horizontal, QObject::tr("标号"));
        model->setHeaderData(1, Qt::Horizontal, QObject::tr("脚本"));
    ...
    while ((bytes = scriptFile.readLine()), bytes.size() > 0)
    {
        ...
        if (splitted.size() != 2)
        {
            msgBox.setText(QObject::tr("脚本格式错误"));
            msgBox.exec();
    ...
    {
        msgBox.setText(QObject::tr("加载脚本出错: %d").arg(nextLabel));
        msgBox.exec();
    }
}
...
void MainWindow::on_pushButton_ScriptStop_clicked()
{
    ...
    ui->pushButton_ScriptExec->setText(QObject::tr("执行"));
    on_scriptLabel(0);
...
{
    if (ui->pushButton_ScriptExec->text() == QObject::tr("执行"))
    {
        //
        // 执行
        ...
        ui->pushButton_ScriptExec->setText(QObject::tr("暂停"));
    }
    else
    {
        //
